...
meme_url = 'https://github.com/Jade9ja/AspiritusDiscord/blob/master/img/spooderman.jpg?raw=true'
...
class management(commands.Cog):
    ...
    @commands.command()
    async def ruadmin(self, ctx):
        """ Bruh, u iz admeme/owner? """
        if ctx.author.id in self.config.owners:
            return await ctx.send(f"Hello admeme/owner **{ctx.author.name}** senpai!😊 ")
        if ctx.author.id == 464316695964811267:
            async with aiohttp.ClientSession() as session:
                async with session.get(meme_url) as resp:
                    if resp.status != 200:
                        return await ctx.send('Could not download file...')
                    data = io.BytesIO(await resp.read())
                    await ctx.send(file=discord.File(data, 'spooderman.jpg'))
        await ctx.send(f"Just yeeet off...😒 {ctx.author.name}")
    ...
    async def dm(self, ctx, user_id: int, *, message: str):
        ...
        try:
            ...
            await ctx.send("This user might be having DMs blocked or it's a client account...")
